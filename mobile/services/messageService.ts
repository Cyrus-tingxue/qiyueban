...
const emitMessageDataChanged = (type: string) => {
  eventBus.emit(Events.MESSAGE_DATA_CHANGED, { type });
};
...
export const messageService = {
  ...
  getMessages: async (otherUserId: number, skip = 0, limit = 50) => {
  ...
  getUnreadOverview: async () => {
    ...
    return {
      ...
      private_unread_count: privateConversations.reduce((sum: number, item: any) => sum + (item.unread_count || 0), 0),
      group_unread_count: groups.reduce((sum: number, item: any) => sum + (item.unread_count || 0), 0),
      conversations: privateConversations,
      groups,
    };
  },

  createGroup: async (name: string, memberIds: number[] = [], isPublic: boolean = true) => {
  ...
  getGroupMessages: async (groupId: number, skip = 0, limit = 100) => {
